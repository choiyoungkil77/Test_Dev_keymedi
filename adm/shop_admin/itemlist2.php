...
<div class="btn_add01 btn_add">
 <? if($is_admin) { ?>
	<a href="./itemexcel_down2.php?m_class=<?=$m_class?>">엑셀다운로드</a>
<? } ?>
	<a href="./itemexcel_up.php" onclick="return excelform(this.href);" target="_blank">상품일괄수정</a>
	 
   <a href="./itemexcel2.php" onclick="return excelform(this.href);" target="_blank">상품일괄등록</a>
</div>
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr>
		...
		<th scope="col" rowspan="3">업체명</th>
		
        <th scope="col" rowspan="3"><?php echo subject_sort_link('it_id', 'sca='.$sca); ?>마스터코드</a></th>
		<?if($is_admin){?>
		<th scope="col" rowspan="3">셀러상품코드</th>
		<? } ?>
		<th scope="col" rowspan="3">업체코드</th>

		<th scope="col" rowspan="3"><?php echo subject_sort_link('it_use', 'sca='.$sca, 1); ?>승인</a><br><input type="checkbox" name="chkall2" value="1" id="chkall2" onclick="check_all2(this.form)"></th>
        <th scope="col" <? if($is_admin){?>colspan="5"<?}else{?>colspan="4"<?}?>>분류</th>
		
		<th scope="col" rowspan="3">규격</th>
		<th scope="col" rowspan="3">단위</th>
		<th scope="col" rowspan="3"><?php echo subject_sort_link('it_maker', 'sca='.$sca); ?>제조사</a></th>

		<th scope="col" rowspan="3">최소금액</th>

		<th scope="col" rowspan="3">표준코드</th>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td <? if($is_admin){?>colspan="5"<?}else{?>colspan="4"<?}?>>
            ...
            <select name="ca_id[<?php echo $i; ?>]" id="ca_id_<?php echo $i; ?>" <? if(!$is_admin) {?>disabled<? }?>>
            ...
            <select name="ca_id2[<?php echo $i; ?>]" id="ca_id2_<?php echo $i; ?>" <? if(!$is_admin) {?>disabled<? }?>>
            ...
            <select name="ca_id3[<?php echo $i; ?>]" id="ca_id3_<?php echo $i; ?>" <? if(!$is_admin) {?>disabled<? }?>>
        ...
        <td rowspan="3" class="td_mng">
			<? if($row['it_8']==""){ ?>
			<a href="./itemcopy.php?it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>" class="itemcopy" target="_blank"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>상품등록</a>
			<? } ?>
			<? if($is_admin) { ?>
			...
			<a href="./itemform2.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>수정</a>
			<? } ?>
