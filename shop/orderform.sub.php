...
<div id="sod_frm">
    <!-- 주문상품 확인 시작 { -->
	<p>※ 결제안내사항</p>
	<p style="color:red">- 급여의약품 포함한 공급사에는 쿠폰, 포인트를 적용할 수 없습니다.</p>
	<p>- 쿠폰,포인트 사용 후 단순변심에 의한 취소 및 반품건은 쿠폰과 포인트가 반환되지 않습니다.</p>
    <p>주문하실 상품을 확인하세요.</p>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        $sql = " select 
						(select mb_nick from {$g5['member_table']} where mb_id = b.it_10 ) as mb_nick ,
						a.ct_id,
                        a.it_id,
                        a.it_name,
                        a.ct_price,
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
			...
			$add_row = sql_fetch("select it_5 , it_6 from {$g5['g5_shop_item_table']} where it_id = '{$row['it_8']}' ");
...
?>
		...
		<tr>
			...
			포인트사용 : <input type="number" step="100" min="0" max="<?=$member['mb_point']?>" maxlength="7" size="7" class="frm_input"  id="f_point[]" value="0" name="f_point[]" onchange="add_point(this,<?=$sell_total_price?>,this.value,'<?=$coupon_com_id?>',<?=$sell_total_price + get_sendcost($com_od_id)?>);return false;">점
			</td>
		</tr>
		<? }else{ ?>
...
<? if($member['mb_point'] > 0){ ?>
	if( val > <?=$member['mb_point']?>  ){
		alert("보유포인트보다 크게 입력할수 없습니다.");	
		ttmp.value = "0";
		document.getElementById("tmp_f_point_"+codes1).value = 0;
		document.getElementById("f_point_price"+codes1).innerHTML = "- 포인트 : 0";
		...
		document.getElementById("f_point_price"+codes1).innerHTML = "- 포인트 : "+val;
