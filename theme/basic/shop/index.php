		...
		<div class="mContents">
			...
			for($i=0;$row = sql_fetch_array($res);$i++){ ?>
				...
				<dl class="<?if($i==0){ echo "first"; }?>">
					<dt class="pic"><a href="/shop/search.php?q=<?=$row[it_id]?>&q_where=main"><?if(is_file(G5_PATH."/data/item/".$row[it_img1])){?><img src="/data/item/<?=$row[it_img1]?>" alt="" width="150" height="110"/><?}else{?><img src="/shop/img/no_image.gif" width="150" height="110" ><?}?></a></dt>
...
<div id="main_wrap">
	<div id="main_01">
		<h4 class="main_titlie">베스트상품111</h4>
		<?
		...
		*/
		$res = sql_query($sql);
		for($i=0;$row = sql_fetch_array($res);$i++){ ?>
			<? $pri = sql_fetch("select it_price , it_img1 , it_5 , it_6 ,it_id from shop.{$g5['g5_shop_item_table']} where  it_use = '1' and it_8 = '{$row[it_id]}' order by it_price asc limit 1");?>
		<dl class="new_item_first <?if($i==0){ echo "first"; }?>">
			<dt class="pic"><a href="/shop/search.php?q=<?=$row[it_id]?>&q_where=main"><?if($row[it_img1]){?><img src="/data/item/<?=$row[it_img1]?>" alt="" width="200" height="200"/><?}?></a></dt>
	...
	<div id="main_03_wrap">
		<div id="main_03">
			<ul>
			<?
				$sql = "select * from shop.g5_shop_banner where bn_position = '공통메인_이벤트' and ( bn_device = 'pc' or bn_device = 'both') and bn_begin_time <= now() and bn_end_time >= now() limit 3";
				$res = sql_query($sql);
				while($row = sql_fetch_array($res)){
			?>
				<li class="fL" style="margin-right:5px"><a href="<?=$row[bn_url]?>" target="_blank"><img src="http://obgys.keymedi.com/data/banner/<?=$row[bn_id]?>" alt="" width="290" height="290" /></a></li>
			<? } ?>
			</ul>
			<div id="main_0304">
	...
	<div id="main_02">
		...
		<div class="main_02_itembox" id="main_02_item_1">
			...
			<dl>
				<dt class="pic"><a href="/shop/search.php?q=<?=$row[it_id]?>&q_where=main"><?if(is_file(G5_PATH."/data/item/".$row[it_img1])){?><img src="/data/item/<?=$row[it_img1]?>" alt="" width="200" height="200"/><?}else{?><img src="/shop/img/no_image.gif" width="200" height="200" ><?}?></a></dt>
	...
	<div id="main07">
		...
		<?
			$today = date("Y-m-d");
			$totime = date("H:i");
			$sql = "select * from portal.g5_write_0101 where wr_is_comment = 0 and wr_2 < '$today' and wr_4 != '' and wr_8 = '' and wr_12 = '' and wr_30 = '' order by wr_datetime desc limit 5";
			$res = sql_query($sql);
		?>
			<ul>
				<?
				...
				<li id="medi_list_id_3<?=$i?>">
					<dl>
						<dt><a href="http://keymedi.com/bbs/board.php?bo_table=0101&wr_id=<?=$row[wr_id]?>&type=4" target="_blank"><img src="http://keymedi.com/data/file/0101/<?=$sql_f[bf_file]?>" width="175" height="100" alt="" ></a></dt>
						<dd class="live_hit" id="live_hit_3<?=$i?>"><img src="/img/new_main/view_icon.png" alt="" />&nbsp;<?=$row[wr_hit]?></dd>
						<dd class="live_subject"><a href="http://keymedi.com/bbs/board.php?bo_table=0101&wr_id=<?=$row[wr_id]?>&type=4" target="_blank"><?=$row[wr_subject]?></a></dd>
